...
use pallet_plasm_rewards::{
    ...
    EraIndex, ForceEra, Forcing, CurrentEra, ActiveEra, ActiveEraInfo
};
...
impl<T: Trait> Module<T> {
	...
	fn active_era() -> Option<ActiveEraInfo<MomentOf<T>>> {
		pallet_plasm_rewards::ActiveEra<T>::get()
	}
	...
	fn end_era(active_era: ActiveEraInfo<MomentOf<T>>, _session_index: SessionIndex) {
		...
		if let Some(active_era_start) = active_era.start {
			let now_as_millis_u64 = T::Time::now().saturated_into::<u64>();

            let era_duration = now_as_millis_u64 - active_era_start.saturated_into::<u64>();
            let untreated_era: EraIndex = active_era.index;
			let max_payout = match T::ForSecurityEraReward::get(&untreated_era) {
				Some(reward) => {
					...
					Perbill::from_rational_approximation(1, validator_len as u32 ) * reward
				},
				None => {
					frame_support::print("Error: start_session_index must be set for current_era");
					return;
            ...
            let total_imbalance =  <PositiveImbalanceOf<T>>::zero();
			let validator_payout = total_imbalance.peek();
